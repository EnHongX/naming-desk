function buildSystemPrompt(preferences, glossaryTerms) {
  let basePrompt = `你是一个专业的程序员命名助手。用户会输入中文描述，你需要根据描述生成符合编程规范的英文命名。
...
async function generateNamingWithAIWithContext(input, preferences, glossaryTerms) {
  const apiKey = process.env.DASHSCOPE_API_KEY;
  const model = process.env.QWEN_MODEL || 'qwen3.5-flash';
  const baseUrl = process.env.DASHSCOPE_BASE_URL || 'https://dashscope.aliyuncs.com/compatible-mode/v1';
  ...
  const systemPrompt = buildSystemPrompt(preferences, glossaryTerms);
...
export { generateNamingWithAI, generateNamingWithAIWithContext };
